...
##correlation matrix for variables before stepwise selection
```{r}
require(mosaic)
...
full_model <- lm(proficiency_index ~ english_language_learner+disabilities+economically_disadvantaged+females+avg_class_size+avg_expenditures_pupil+proficiency_index+avg_salary+white, data = pruned_ma_schools)
...
```{r}
# FINAL MODEL
model <- lm(proficiency_index ~ economically_disadvantaged + disabilities + 
    avg_expenditures_pupil + english_language_learner + 
    females + avg_class_size, data = pruned_ma_schools)
