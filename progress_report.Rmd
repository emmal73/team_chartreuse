...
##multicollinearity before stepwise selection
```{r}
require(mosaic)
stepwise_ma_schools <- pruned_ma_schools %>%
  select_("X..English.Language.Learner","X..Students.With.Disabilities","X..Economically.Disadvantaged","X..White","X..Females","Average.Class.Size","Average.Salary","Average.Expenditures.per.Pupil","proficiency_index")
cor1<-cor(stepwise_ma_schools)
cor1
pairs(~X..English.Language.Learner + X..Students.With.Disabilities + X..Economically.Disadvantaged + X..White + X..Females + Average.Class.Size + Average.Salary + Average.Expenditures.per.Pupil+proficiency_index, data = pruned_ma_schools, main = "Simple Scatterplot Matrix")

```

## Stepwise Selection(finalModel)
```{r}
...
```{r}
model <- lm(proficiency_index ~ X..Economically.Disadvantaged + X..Students.With.Disabilities + 
    Average.Expenditures.per.Pupil + X..English.Language.Learner + 
    X..Females + Average.Class.Size, data = pruned_ma_schools)

summary(model)
```

##correlation matrix for final model
```{r}
final_ma_schools <- pruned_ma_schools %>%
  select_("X..English.Language.Learner","X..Students.With.Disabilities","X..Economically.Disadvantaged","X..Females","Average.Class.Size","Average.Expenditures.per.Pupil","proficiency_index")
cor2<-cor(final_ma_schools)
cor2
```
